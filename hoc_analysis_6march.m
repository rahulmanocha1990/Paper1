%% --single trial classificatin of hoc features
cd ../Paper1/FinalFeat
Direc=dir('*.mat');
...
%%
ExperimentId='../Paper1/Exp_7_March_2015_hoc_nb_svm.csv';
fid=fopen(ExperimentId,'w');
fprintf(fid,'Single Trial Classification with HOC features \n');
Nf=20;
...
for filenum=1:size(FeatFiles,1)
  ...
  load(name,'HocFeat','labels');
  FeatVect=[];
  for vid=1:40
      A=squeeze(mean(HocFeat{vid}));
      A=A(:)';
      FeatVect=[FeatVect;A];
  end   
  Valence=labels(:,1)>5;

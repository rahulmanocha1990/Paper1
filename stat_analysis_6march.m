...
%%
ExperimentId='../Paper1/Exp_7_March_2015_stat_nb_svm.csv';
fid=fopen(ExperimentId,'w');
fprintf(fid,'Single Trial Classification with STAT features \n');
Nf=20;
...
for filenum=1:size(FeatFiles,1)
  ...
  load(name,'StatFeat','labels');
  FeatVect=[];
  for vid=1:40
      A=squeeze(mean(StatFeat{vid}));
      A=A(:)';
      FeatVect=[FeatVect;A];
  end   
  Valence=labels(:,1)>5;

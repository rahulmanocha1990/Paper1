...
%%
ExperimentId='../Paper1/Exp_24_Feb_2015_class4_allfeat.csv';
...
for filenum=1:size(FeatFiles,1)
   ...
   mcrindex=1;
   for kfold=1:10
       if(stats.per_fold(kfold).mcr < mcr)%1.1*loss && stats.per_fold(kfold).mcr > 0.90*loss)
           mcrindex=kfold;
           mcr=stats.per_fold(kfold).mcr;
       end
   end
   Pred=stats.per_fold(mcrindex).pred{1,2};
   Pred_labels=(Pred(:,1)+Pred(:,2)*2+Pred(:,3)*3+Pred(:,4)*4)-1;
   C=confusionmat(stats.per_fold(mcrindex).targ,Pred_labels);
...
%%
ExperimentId='../Paper1/Exp_24_Feb_2015_class4_hoc.csv';
fid=fopen(ExperimentId,'w');
...
%%
ExperimentId='../Paper1/Exp_24_Feb_2015_class4_stat.csv';
fid=fopen(ExperimentId,'w');
fprintf(fid,'Paper5 4classes  PLL PHH NLL NHH and features [HOC] \n');
...
%%
ExperimentId='../Paper1/Exp_24_Feb_2015_class4_hoc_stat.csv';
fid=fopen(ExperimentId,'w');
fprintf(fid,'Paper5 4classes  PLL PHH NLL NHH and features [HOC] \n');

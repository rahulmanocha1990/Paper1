...
%%
for filenum=1:size(FeatFiles,1)
     name=sprintf('../bcilab_matfiles/%s',FeatFiles(filenum,:));
     EEG=load(name);
     count=1;
     for nevent=1:40
       handl=figure;
       [spectra,freqs]=spectopo(EEG.data(:,EEG.event(nevent).latency:8063+EEG.event(nevent).latency),0,128,...

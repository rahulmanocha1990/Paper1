...
for filenum=1:size(MatFiles,1)
  ...
  outfile=sprintf('../Paper1/Features_4class_stat/%s.mat',name(16:end-4));
  save(outfile,'FC0','FC1','FC2','FC3');
...
% 
% %%
% ExperimentId='../Paper1/Exp_18_Feb_2015_class4_stat.csv';
% fid=fopen(ExperimentId,'w');
...
for filenum=1:size(MatFiles,1)
  ...
  for i=1:length(VideoClass0)
     X=squeeze(data(VideoClass0(i),:,epoch(1):epoch(2)));
     FC0=[FC0;FeatureGen(X,Cind,wlen,wshft,Fs,'HOC',5)];
  end 

  for i=1:length(VideoClass1)
     X=squeeze(data(VideoClass1(i),:,epoch(1):epoch(2)));
     FC1=[FC1;FeatureGen(X,Cind,wlen,wshft,Fs,'HOC',5)];
  end
  for i=1:length(VideoClass2)
     X=squeeze(data(VideoClass2(i),:,epoch(1):epoch(2)));
     FC2=[FC2;FeatureGen(X,Cind,wlen,wshft,Fs,'HOC',5)];
  end 

  for i=1:length(VideoClass3)
     X=squeeze(data(VideoClass3(i),:,epoch(1):epoch(2)));
     FC3=[FC3;FeatureGen(X,Cind,wlen,wshft,Fs,'HOC',5)];
  end
  
  
  outfile=sprintf('../Paper1/Features_4class_HOC/%s.mat',name(16:end-4));
  save(outfile,'FC0','FC1','FC2','FC3');
